...
public interface ProductRepoitory extends JpaRepository<ProductEntity, Long> {
    ...
    @Query(value = "select p.* from product p where p.product_price > :productPrice", nativeQuery = true)
    ...
    @Query(value ="select p.* from product p where p.product_price > :productPrice"
    , countQuery = "select count(*) from product p where p.product_price > ?1"
    ,nativeQuery = true)
    List<ProductEntity> findByproductPriceWithParameterPaging(Integer productPrice, Pageable pageable);
